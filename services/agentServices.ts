...
export default class AgentService {
  /* ───────────────────────────────────────────────
     RF03.1 – Registrar agente usando el token
  ────────────────────────────────────────────────*/
  static async registerWithToken(data: {
    ...
    try {
      // 1️⃣ Verifica que el email no exista
      ...
      // 2️⃣ Valida el token y obtiene realEstateId
      ...
      // 3️⃣ Hashea la contraseña
      const hashedPassword = await bcrypt.hash(data.password, 10);

      // 4️⃣ Crea instancia del agente
      ...
      // 5️⃣ Registra en la base de datos
  ...
  /* ───────────────────────────────────────────────
     RF03.2 – Solicitud manual de ingreso
  ────────────────────────────────────────────────*/
  static async requestJoin(data: {
    ...
    await UserRepository.insertAgentJoinRequest(
      name_person,
      last_name,
      email,
      phone,
  ...
  /* ───────────────────────────────────────────────
     RF03.6 – Cancelar solicitud (por agente)
  ────────────────────────────────────────────────*/
